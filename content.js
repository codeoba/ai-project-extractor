// AI Project Extractor - Power Developer Suite Content Script (Final Ultimate Edition)
...
(function () {
  ...
  // -------------------------------------------------------------------
  // FEATURE 1: AUTO README GENERATOR
  // -------------------------------------------------------------------

  function generateReadmeMD() {
    const projectName = document.getElementById('project-name-input').value || 'ai-project';
    
    // Check if README.md already exists, remove it first to overwrite
    detectedFiles = detectedFiles.filter(f => f.name.toLowerCase() !== 'readme.md');

    let treeMapStr = "";
    detectedFiles.forEach(f => {
      treeMapStr += `- **${f.name}** (${getExtensionFromLang(f.name).toUpperCase()} script)\n`;
    });

    const isDocker = detectedFiles.some(f => f.name.includes('docker-compose'));
    const isPy = detectedFiles.some(f => f.name.endsWith('requirements.txt'));
    const isNode = detectedFiles.some(f => f.name.endsWith('package.json'));

    let instructions = "### Running locally:\nRun the auto-generated `setup.bat` (Windows) or `setup.sh` (Linux/Mac) to instantly provision local development dependencies and launch.";
    if (isDocker) {
      instructions = "### Container Devops Launch:\nRun the following docker command to launch in isolated environments:\n```bash\ndocker compose up --build\n```";
    }

    const readmeContent = `# ${projectName}\n\nAutogenerated project extracted via **AI Project Extractor** Chrome Extension.\n\n## Project Directory Tree\n${treeMapStr}\n\n## Deployment & Running Instructions\n${instructions}\n`;
  ...
  // -------------------------------------------------------------------
  // FEATURE 3: INTERACTIVE REGEX TESTER SANDBOX
  // -------------------------------------------------------------------

  function runRegexSandbox() {
    const regexInput = document.getElementById('custom-regex-input').value;
    const testText = document.getElementById('regex-test-text').value;
    const resultBox = document.getElementById('regex-test-result');

    if (!regexInput || !testText) {
      resultBox.innerText = "Please input a custom regex and paste test text.";
    ...
    try {
      ...
      if (matches.length === 0) {
        resultBox.innerText = "No filename matches found using this regex pattern.";
        resultBox.style.color = "#ef4444";
      } else {
        resultBox.innerHTML = `Matched files (${matches.length}): <br>${matches.map(m => `<code>${cleanFilename(m)}</code>`).join(', ')}`;
        resultBox.style.color = "#10b981";
      }
    } catch(err) {
      resultBox.innerText = `Regex error: ${err.message}`;
      resultBox.style.color = "#ef4444";
    }
  }

  // -------------------------------------------------------------------
  // FEATURE 4: EXPORT TO CODEPEN
  // -------------------------------------------------------------------

  function exportToCodePen() {
    const htmlFile = detectedFiles.find(f => f.name.endsWith('.html')) || { content: "" };
    const cssFile = detectedFiles.find(f => f.name.endsWith('.css')) || { content: "" };
    const jsFile = detectedFiles.find(f => f.name.endsWith('.js')) || { content: "" };

    const data = {
      title: document.getElementById('project-name-input').value || 'AI Extracted Project',
  ...
  function createUI() {
    ...
    sidebarEl.innerHTML = `
      ...
      <div class="project-info">
        ...
        <div class="extra-actions-row">
          <button id="readme-gen-btn" class="sub-btn" disabled>📝 README</button>
          <button id="codepen-btn" class="sub-btn" disabled>🚀 CodePen</button>
        </div>
      </div>

      <!-- Settings Section with Regex Tester Sandbox -->
      <div class="settings-section">
        ...
        <div id="settings-content" class="settings-content hidden">
          ...
          <div class="settings-group">
            <label>Regex Tester Sandbox:</label>
            <textarea id="regex-test-text" placeholder="Paste test chat output text..."></textarea>
            <div id="regex-test-result" class="status-msg">Sandbox ready.</div>
          </div>
          <div class="settings-group">
      ...
      <!-- Selective packaging subfolder filter -->
      <div class="sync-section">
        <div class="section-title">Selective Folder packaging</div>
        <div class="settings-group">
          <input type="text" id="zip-folder-filter" placeholder="Zip only: e.g. libs/ (optional)">
        </div>
      </div>
      ...
      <div class="sidebar-footer">
        <span>AI Project Extractor v1.4</span>
      </div>
    `;
    ...
    document.getElementById('readme-gen-btn').addEventListener('click', generateReadmeMD);
    document.getElementById('codepen-btn').addEventListener('click', exportToCodePen);
    ...
    // Regex Sandbox trigger listeners
    document.getElementById('custom-regex-input').addEventListener('input', runRegexSandbox);
    document.getElementById('regex-test-text').addEventListener('input', runRegexSandbox);
  ...
  function renderFileList() {
    ...
    // Enable CodePen if HTML file is present
    codepenBtn.disabled = !detectedFiles.some(f => f.name.endsWith('.html'));
  ...
  // -------------------------------------------------------------------
  // CODE EDITOR & DIFF VISUALIZER & LINTER & SEARCH/REPLACE & FORMATTER
  // -------------------------------------------------------------------

  function formatCodeContent(content, extension) {
    const ext = extension.toLowerCase();
    
    // Feature 2: In-Editor code formatter
    if (ext === 'json') {
      try {
    ...
    // Basic indentation formatter fallback for other formats
    let formatted = '';
    let indent = 0;
  ...
  function showPreviewModal(file) {
    ...
    modal.innerHTML = `
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button id="modal-format-btn" class="sub-btn">✨ Format Code</button>
    ...
    // Formatting button trigger
    modal.querySelector('#modal-format-btn').addEventListener('click', () => {
      textEditor.value = formatCodeContent(textEditor.value, file.name.split('.').pop());
      runLinter();
    });
    ...
    aiRunBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        aiOutputText.value = `AI Generation failed: ${err.message}\n\nMake sure to add a valid Gemini Key in the settings panel.`;
      } finally {
  ...
  // -------------------------------------------------------------------
  // ZIP COMPRESSION WITH SUBFOLDER FILTERS
  // -------------------------------------------------------------------
  ...
  function exportAsZip() {
    ...
    const projectNameInput = document.getElementById('project-name-input');
    const projectName = projectNameInput.value.trim() || 'ai-project';
    const folderFilter = document.getElementById('zip-folder-filter').value.trim();

    let filesToPack = detectedFiles;
    if (folderFilter) {
      filesToPack = detectedFiles.filter(f => f.name.startsWith(folderFilter));
    }

    if (filesToPack.length === 0) {
      alert("No files match the folder pack filter.");
      return;
    }

    filesToPack.forEach(file => {
      // Strip the filtered folder prefix to pack nicely
      let zipPath = file.name;
      if (folderFilter && zipPath.startsWith(folderFilter)) {
        zipPath = zipPath.replace(folderFilter, '').replace(/^\//, '');
      }
    ...
    zip.generateAsync({ type: 'blob' }).then((content) => {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `${projectName}${folderFilter ? '-' + folderFilter.replace(/[\/\\:*?"<>|]/g, '') : ''}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
